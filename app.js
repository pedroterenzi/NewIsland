...
async function executarLogin() {
    ...
    try {
        ...
        alert("Erro ao conectar no servidor. Verifique se o Render está online.");
    } finally {
...
async function carregarDadosMestres() {
    try {
        const resSkus = await fetch(`${API_URL}/skus`);
        if (resSkus.ok) DADOS_SKUS = await resSkus.json();

        const resParadas = await fetch(`${API_URL}/paradas-codigos`);
        if (resParadas.ok) DADOS_CODIGOS_PARADA = await resParadas.json();
    } catch (e) {
