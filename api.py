...
def processar_apontamento(dados: PayloadApontamento, cursor, turno_id=None):
    ...
    if turno_id:
        ...
        # Limpa filhos antigos para recriar
...
def listar_lancamentos(data: str = None, turno: int = None, maquina: int = None, ordem: str = None):
    try:
        conn = psycopg2.connect(DATABASE_URL)
        cursor = conn.cursor(cursor_factory=RealDictCursor)
        
        query = "SELECT r.id, r.data_registro::text, r.turno, r.operador, r.maquina_numero, (SELECT SUM(machine_counter) FROM result_by_order WHERE turno_id = r.id) as total_mc FROM registro_turnos r WHERE 1=1 "
        params = []
...
def visao_ordens():
    try:
        ...
        query = """
            ...
            GROUP BY ro.ordem, ro.codigo_sku ORDER BY ro.ordem DESC;
        """
