...
def obter_dados_mestres():
    # Agora a rota exige que o banco funcione, senão dispara erro pro app saber que falhou!
    try:
        conn = psycopg2.connect(DATABASE_URL)
        cursor = conn.cursor(cursor_factory=RealDictCursor)
        ...
        raise HTTPException(status_code=500, detail="Banco de dados indisponível no momento.")
