...
def db_execute(query, params=None, is_select=False):
    conn = None
    try:
        conn = psycopg2.connect(DATABASE_URL)
        cursor = conn.cursor(cursor_factory=RealDictCursor if is_select else None)
        if params: cursor.execute(query, params)
        else: cursor.execute(query)
        if is_select: res = cursor.fetchall()
        else: conn.commit(); res = {"status": "sucesso"}
        return res
    except Exception as e:
        if conn and not is_select: conn.rollback()
        raise HTTPException(status_code=500, detail=str(e))
    finally:
        if conn:
            cursor.close()
...
def h_l(): return db_execute("SELECT r.id, r.data_registro::text, r.turno, r.operador, r.maquina_numero, (SELECT COALESCE(SUM(machine_counter),0) FROM result_by_order WHERE turno_id = r.id) as total_mc FROM registro_turnos r ORDER BY r.data_registro DESC, r.turno ASC LIMIT 100;", is_select=True)
@app.delete("/historico-lancamentos/{id}")
def d_l(id: int): return db_execute("DELETE FROM registro_turnos WHERE id=%s;", (id,))
