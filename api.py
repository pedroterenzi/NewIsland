...
def inicializar_banco():
    try:
        # Usa o 'with' para garantir que a conexão fecha INSTANTANEAMENTE após terminar
        with psycopg2.connect(DATABASE_URL) as conn:
            with conn.cursor() as cursor:
                # 1. Checa se o schema está na versão nova
                ...
                # 2. Cria as tabelas limpas (Sem injeção forçada de carga gigante para não estourar tempo)
            ...
            conn.commit()
            print("⚡ Banco Inicializado e Limpo sem vazamento de conexões!")
...
def ed(id: int, m: ModelMaquina):
    try:
        with psycopg2.connect(DATABASE_URL) as conn:
            with conn.cursor() as c: c.execute("UPDATE maquinas SET numero_maquina=%s, tipo=%s, ativo=%s WHERE id=%s;", (m.numero_maquina, m.tipo, m.ativo, id)); conn.commit()
        return {"status": "sucesso"}
    except Exception as e: raise HTTPException(status_code=500, detail=str(e))
...
def ed_tno(id: int, t: ModelTNOMestre):
    try:
        with psycopg2.connect(DATABASE_URL) as conn:
            with conn.cursor() as c: c.execute("UPDATE tipos_tno SET nome=%s WHERE id=%s;", (t.nome, id)); conn.commit()
        return {"status": "sucesso"}
    except Exception as e: raise HTTPException(status_code=500, detail=str(e))
...
def del_tno(id: int):
    try:
        with psycopg2.connect(DATABASE_URL) as conn:
            with conn.cursor() as c: c.execute("DELETE FROM tipos_tno WHERE id=%s;", (id,)); conn.commit()
        return {"status": "sucesso"}
    except Exception as e: raise HTTPException(status_code=500, detail=str(e))
...
def ed_p(id: int, p: ModelParadaMestre):
    try:
        with psycopg2.connect(DATABASE_URL) as conn:
            with conn.cursor() as c: c.execute("UPDATE codigos_parada SET tipo_maquina=%s, numero=%s, problema=%s WHERE id=%s;", (p.tipo_maquina, p.numero, p.problema, id)); conn.commit()
        return {"status": "sucesso"}
    except Exception as e: raise HTTPException(status_code=500, detail=str(e))
...
def del_p(id: int):
    try:
        with psycopg2.connect(DATABASE_URL) as conn:
            with conn.cursor() as c: c.execute("DELETE FROM codigos_parada WHERE id=%s;", (id,)); conn.commit()
        return {"status": "sucesso"}
    except Exception as e: raise HTTPException(status_code=500, detail=str(e))
...
def ed_u(id: int, u: ModelUsuario):
    try:
        with psycopg2.connect(DATABASE_URL) as conn:
            with conn.cursor() as c: c.execute("UPDATE usuarios SET login=%s, senha=%s, nome=%s, nivel=%s WHERE id=%s;", (u.login.lower(), u.senha, u.nome, u.nivel, id)); conn.commit()
        return {"status": "sucesso"}
    except Exception as e: raise HTTPException(status_code=500, detail=str(e))
...
def del_u(id: int):
    try:
        with psycopg2.connect(DATABASE_URL) as conn:
            with conn.cursor() as c: c.execute("DELETE FROM usuarios WHERE id=%s;", (id,)); conn.commit()
        return {"status": "sucesso"}
    except Exception as e: raise HTTPException(status_code=500, detail=str(e))
...
def add_sku(s: ModelSKU):
    try:
        with psycopg2.connect(DATABASE_URL) as conn:
            with conn.cursor() as c: c.execute("INSERT INTO master_sku (codigo_sku, descricao, fraldas_por_pacote, pacotes_por_fardo, fardos_por_pallet) VALUES (%s, %s, %s, %s, %s);", (s.codigo_sku, s.descricao, s.fraldas_por_pacote, s.pacotes_por_fardo, s.fardos_por_pallet)); conn.commit()
        return {"status": "sucesso"}
    except Exception as e: raise HTTPException(status_code=500, detail=str(e))
...
def mod_sku(id: int, s: ModelSKU):
    try:
        with psycopg2.connect(DATABASE_URL) as conn:
            with conn.cursor() as c: c.execute("UPDATE master_sku SET codigo_sku=%s, descricao=%s, fraldas_por_pacote=%s, pacotes_por_fardo=%s, fardos_por_pallet=%s WHERE id=%s;", (s.codigo_sku, s.descricao, s.fraldas_por_pacote, s.pacotes_por_fardo, s.fardos_por_pallet, id)); conn.commit()
...
def del_sku(id: int):
    try:
        with psycopg2.connect(DATABASE_URL) as conn:
            with conn.cursor() as c: c.execute("DELETE FROM master_sku WHERE id = %s;", (id,)); conn.commit()
        return {"status": "removido"}
    except Exception as e: raise HTTPException(status_code=500, detail=str(e))
...
def add_maq(m: ModelMaquina):
    try:
        with psycopg2.connect(DATABASE_URL) as conn:
            with conn.cursor() as c: c.execute("INSERT INTO maquinas (numero_maquina, tipo, ativo) VALUES (%s, %s, %s);", (m.numero_maquina, m.tipo, m.ativo)); conn.commit()
        return {"status": "sucesso"}
    except Exception as e: raise HTTPException(status_code=500, detail=str(e))
...
def add_tno(t: ModelTNOMestre):
    try:
        with psycopg2.connect(DATABASE_URL) as conn:
            with conn.cursor() as c: c.execute("INSERT INTO tipos_tno (nome) VALUES (%s);", (t.nome,)); conn.commit()
        return {"status": "sucesso"}
    except Exception as e: raise HTTPException(status_code=500, detail=str(e))
...
def add_p(p: ModelParadaMestre):
    try:
        with psycopg2.connect(DATABASE_URL) as conn:
            with conn.cursor() as c: c.execute("INSERT INTO codigos_parada (tipo_maquina, numero, problema) VALUES (%s, %s, %s);", (p.tipo_maquina, p.numero, p.problema)); conn.commit()
        return {"status": "sucesso"}
    except Exception as e: raise HTTPException(status_code=500, detail=str(e))
...
def add_u(u: ModelUsuario):
    try:
        with psycopg2.connect(DATABASE_URL) as conn:
            with conn.cursor() as c: c.execute("INSERT INTO usuarios (login, senha, nome, nivel) VALUES (%s, %s, %s, %s);", (u.login, u.senha, u.nome, u.nivel)); conn.commit()
        return {"status": "sucesso"}
    except Exception as e: raise HTTPException(status_code=500, detail=str(e))
